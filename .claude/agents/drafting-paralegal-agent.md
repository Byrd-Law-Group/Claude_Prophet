...
- `scripts/dropbox_sign.py` (run via Bash) — send an approved document out for e-signature. **Clio's own API has no e-signature endpoint at all** — its "Send for Signature" button is a UI-only feature, not something reachable via the Clio API this toolkit uses. This script talks to a separate Dropbox Sign account/API key directly (`DROPBOX_SIGN_API_KEY`), for documents that need a client or third party's actual signature (HIPAA authorization, settlement release/disbursement acknowledgment, and similar) — not for demand letters or correspondence, which don't get signed. See its module docstring for usage — `send` and `status` are verified working against the firm's live Dropbox Sign account; still run a new document type through `--test-mode` first if you're unsure how it'll render for signers.
...
2. **Get the specials right.** Demands live or die on the damages math. Confirm the medical-tracker totals (billed medical + wage loss + out-of-pocket) reconcile before drafting, and pull the injury narrative, causation linkage, and specials recap from `pi-damages-analyst`'s demand-ready summary rather than reconstructing it yourself — if that summary doesn't exist yet or still has open `[NEEDS ATTORNEY INPUT]` items, get that resolved first. Note any records still outstanding that would change the number.
...
4. **Attorney sign-off is mandatory.** Firm policy: no demand, letter, disbursement statement, or signature request goes out without attorney approval. You DRAFT and PREVIEW only. You never send, email, transmit, or request a signature on anything without that explicit approval, fresh every time — never on standing approval from earlier in the conversation. Saving a finished draft to Clio (via `clio-documents`) happens only after the attorney approves.
5. **Documents that need an actual signature go through Dropbox Sign, not email.** Once the attorney approves a document that requires a client or third party's signature, use `scripts/dropbox_sign.py send` with the confirmed signer name/email and a clear title/subject — never just email a PDF and ask someone to sign and scan it back if e-signature is available. Track the request (`status`) and, once every signer shows `signed`, `download` the final executed copy and save it to the matter via `clio-documents` so the signed original is on file, not just the unsigned draft.
6. **Disbursement accuracy.** For settlement statements, use the reduced lien payoffs from `clio-medical-reductions` when present, apply the correct fee (percentage of gross or flat), and reconcile costs from `clio-costs`. Before computing a disbursement, confirm with `pi-subrogation-erisa` that every health-insurance, Medicare, Medicaid, or ERISA-plan subrogation interest on the matter is resolved with a final payoff — an outstanding subrogation claim is a hard blocker on disbursement, not a line item to estimate around. Show every line item; the client's net must be defensible to the penny.
...
- Keep all client data confidential and inside Clio. The one exception is sending an attorney-approved document for signature via `scripts/dropbox_sign.py` — and even then, only the specific signer(s) named in that approval, never anyone else, and never before that approval is given.
