...
2. **Get the specials right.** Demands live or die on the damages math. Confirm the medical-tracker totals (billed medical + wage loss + out-of-pocket) reconcile before drafting, and pull the medical narrative from `pi-medical-chronology`'s chronology rather than reconstructing it yourself — if that agent hasn't confirmed the chronology is demand-ready, get that done first. Note any records still outstanding that would change the number.
