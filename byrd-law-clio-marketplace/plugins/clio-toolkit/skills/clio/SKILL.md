...
Uploading is a **3-step flow** — Clio hands back a one-time AWS S3 URL, you PUT the
bytes straight to S3, then you tell Clio the upload is complete. The helper script
does all three; prefer it over hand-rolling the flow.

```bash
python skills/clio/scripts/upload_document.py --file ./demand_letter.pdf --matter 12345
```

Options: `--name "Display Name.pdf"` to override the file name; `--parent-id <id>
--parent-type Folder` to file it inside a folder instead of directly on the matter.
Set `MATON_CONNECTION` to target a specific Clio connection.

The three steps the script performs, if you need to do them manually:

**Step 1 — register the document and get the signed upload URL.** Request the
...
header from `put_headers` exactly as given. This request goes to Amazon, **not** the
Maton gateway, and must **NOT** include the `Authorization: Bearer $MATON_API_KEY`
header — the signed URL is its own auth.

**Step 3 — finalize.** Mark the version complete using the `uuid` from step 1.
Until this runs, the document exists but has no usable content:
...
#### Edit a Document (Upload a New Version)

To replace a document's contents, run the same 3-step flow but pass the existing
document ID — step 1 becomes a `PATCH` (not `POST`) to `/documents/{id}.json`, which
returns a fresh `put_url` for a new version. Clio keeps the prior versions.

```bash
python skills/clio/scripts/upload_document.py --file ./demand_letter_v2.pdf --document 67890
```

#### Update Document Metadata (Rename / Move)

To rename a document or move it to a different matter or folder without changing its
contents, PATCH the metadata (no bytes, no `fully_uploaded`):
...
Or via the helper: `python skills/clio/scripts/upload_document.py --document 67890 --name "Final Demand.pdf"`
...
**Document notes:**
- The `documents` endpoints require the Documents OAuth scope beyond the basic integration.
- `parent.type` is `Matter` to file directly on a matter, or `Folder` for a subfolder.
- The S3 PUT in step 2 must omit the Maton Authorization header and include all `put_headers`.
- A document with no completed step 3 shows in Clio but cannot be downloaded — always finalize.
